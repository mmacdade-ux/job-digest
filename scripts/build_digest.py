...
# Resend free tier (no verified domain) only delivers to the address the
# Resend account was created with — same account as tech-digest, so same
# recipient. See tech-digest/scripts/sources.py for the original note.
...
def render_email(date, new_by_source, manual_items, n):
    """Email-safe version of build_html: inline styles, no JS/CSS-var reliance."""
    if n == 0:
        site_sections = '<p style="color:#5a5a66;font-size:14px">No new postings today.</p>'
    else:
        parts = []
        for site, src_url, _ in SOURCES:
            items = new_by_source.get(site, [])
            ...
            parts.append(
                f'<h2 style="font-size:16px;margin:24px 0 10px;color:#1a1a1f">'
                f'{esc(site)} <span style="font-weight:400;color:#5a5a66;font-size:13px">'
                f'({len(items)} new)</span></h2>{rows}')
        site_sections = "".join(parts)

    manual_lis = "".join(
        f'<li style="margin-bottom:4px"><a href="{esc(u)}" style="color:#2f7fc1">{esc(name)}</a></li>'
        for name, u, _note in manual_items)
    manual_block = (
        '<h2 style="font-size:16px;margin:28px 0 8px;color:#8a6d1f">Check these manually</h2>'
        f'<ul style="margin:0;padding-left:18px;color:#5a5a66;font-size:13px">{manual_lis}</ul>')
...
<div class="wrap" style="background:#f4f4f6;padding:24px 12px;">
  <div class="card" style="max-width:640px;margin:0 auto;background:#ffffff;border:1px solid #d9d9e0;border-radius:12px;padding:28px 26px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;line-height:1.55;">
    <h1 style="font-size:22px;margin:0 0 4px;color:#1a1a1f;">Job digest</h1>
    <div class="muted" style="font-size:13px;color:#8a8a96;margin-bottom:8px;">{date} · {n} new postings</div>
    ...
    <div class="muted" style="font-size:12px;color:#8a8a96;">
      Built automatically from public job-board sources.
    </div>
  </div>
</div>
</body>
...
def main():
    ...
    if dry_run:
        (ROOT / "email-preview.html").write_text(
            render_email(date, new_by_source, manual_items, n))
        print("DRY RUN — archive + email-preview.html written, email NOT sent.")
        return
    if n == 0:
        print("0 new postings — skipping email.")
        return
    send_resend(f"Job digest — {date} — {n} new", render_email(date, new_by_source, manual_items, n))
    print("Email sent.")
